...
do $$ declare demo_business uuid; demo record; begin
  insert into public.businesses(name,business_code,subscription_tier,max_staff_limit,has_cashier_module,is_active)
  values('Mesob Demo Restaurant','MESOB-DEMO','pro',20,true,true)
  on conflict (business_code) do update set name=excluded.name,subscription_tier=excluded.subscription_tier,
    max_staff_limit=excluded.max_staff_limit,has_cashier_module=true,is_active=true
  ...
  loop
    -- Prefer the globally unique phone identity when the demo row already
    -- exists; otherwise reuse the staff number, then insert a new row.
