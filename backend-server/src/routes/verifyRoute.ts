...
interface ExistingVerifiedPayment {
  ticket_id: string;
  evidence_id: string;
  provider: string;
  reference: string;
...
async function findCommittedPayment(
  token: string,
  provider: Provider,
  reference: string,
): Promise<ExistingVerifiedPayment | null> {
  return callServiceRpc<ExistingVerifiedPayment | null>(
    "find_committed_payment",
    {
      p_token: token,
      p_provider: provider,
...
async function recordFailedVerification(params: {
  ...
  try {
    await callServiceRpc("service_record_failed_verification", {
...
async function rejectVerifiedPayment(
  res: Response,
  params: {
    token: string;
    provider: Provider;
...
verifyRouter.use((req: Request, res: Response, next) => {
  ...
  if (req.path === "/verify-and-create") return next();
...
async function handleVerifyAndCreate(req: Request, res: Response): Promise<void> {
  const body = (req.body ?? {}) as VerifyBody;
  ...
  try {
    ...
    if (!isProvider(providerRaw)) {
      throw new ClientError(
        `Invalid provider. Must be one of: ${KNOWN_PROVIDERS.join(", ")}`,
        400,
        "INVALID_PROVIDER",
      );
    }
    ...
    if (
      ...
      Buffer.byteLength(body.receiptImageBase64, "base64") > 1_500_000
    ) {
      throw new ClientError(
        "Compressed receipt image exceeds 1.5 MB.",
        413,
        "RECEIPT_TOO_LARGE",
      );
    }
    ...
    if (provider === "abyssinia") {
      ...
      if (!abyssiniaSuffix) {
        await rejectVerifiedPayment(res, {
          ...
          message:
            "The configured Abyssinia account cannot provide the required five-digit destination suffix.",
        });
        return;
      }
    ...
    const verifiedAmount = positiveAmount(data.amount);
    ...
    const freshness = validateTransactionFreshness(providerDate, {
...
verifyRouter.post("/verify-and-create", handleVerifyAndCreate);
