...
operatorRouter.get("/operator/overview", requireOperator, async (_req, res) => {
  ...
  try {
    const snapshot = await callServiceRpc<Record<string, unknown>>(
      "service_operator_snapshot",
      {},
    );
    let supportCases = snapshot.support_cases;
    try {
      supportCases = await callServiceRpc<unknown[]>(
        "service_operator_list_support_cases",
        {},
      );
    } catch (error) {
      if (!(error instanceof SupabaseRpcError && error.code === "PGRST202")) throw error;
    }
    res.json({
      success: true,
...
operatorRouter.patch("/operator/support/:id", requireOperator, async (req, res) => {
  ...
  try {
    ...
    if (reply) {
      await callServiceRpc("service_operator_reply_support_case", {
        p_operator_email: claims.sub,
