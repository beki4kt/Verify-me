...
import {authoritativeAccountSuffix, authoritativeEthiopianPhone} from './paymentSecurity';
...
async function boundedJson(value:Request|Response,limit=500000):Promise<Json>{
  const reader=value.body?.getReader();if(!reader)throw Error('Empty response');let size=0;const chunks:Uint8Array[]=[];
  while(true){const {done,value:chunk}=await reader.read();if(done)break;size+=chunk.length;if(size>limit){await reader.cancel();throw Error('Response too large');}chunks.push(chunk);}
  const bytes=new Uint8Array(size);let offset=0;for(const chunk of chunks){bytes.set(chunk,offset);offset+=chunk.length;}
  const result=JSON.parse(new TextDecoder().decode(bytes));if(!result||typeof result!=='object'||Array.isArray(result))throw Error('Invalid JSON');return result;
}
export function createDemoVerificationHandler(env:Record<string,string|undefined>,transport:typeof fetch=fetch){
  ...
  return async(req:Request)=>{
    ...
    if(origin){if(!(env.CORS_ALLOWED_ORIGINS||'').split(',').map(x=>x.trim()).includes(origin))return new Response('Origin not allowed',{status:403});headers['Access-Control-Allow-Origin']=origin;headers.Vary='Origin';headers['Access-Control-Allow-Headers']='content-type,apikey';headers['Access-Control-Allow-Methods']='GET,POST,OPTIONS';}
    const reply=(status:number,body:Json)=>new Response(JSON.stringify({...body,demo:true,requestId}),{status,headers});
    ...
    const rpc=async(name:string,params:Json)=>{
      ...
      if(!response.ok)throw Error('Demo database unavailable');return await response.json() as Json;
    };
    try{
      ...
      const provider=body.provider as Provider;const reference=typeof body.reference==='string'?body.reference.trim():'';
      ...
      const input:{reference:string;suffix?:string;phoneNumber?:string}={reference};
      if(provider==='cbe'||provider==='abyssinia'){
        const suffix=authoritativeAccountSuffix(body.receivingAccount,provider==='cbe'?8:5);
        if(!suffix)return reply(400,{success:false,error:'Enter the receiving bank account shown on the receipt.'});input.suffix=suffix;
      }
      if(provider==='cbebirr'){
        const phone=authoritativeEthiopianPhone(body.receivingAccount);if(!phone)return reply(400,{success:false,error:'Enter the receiving CBE Birr phone number.'});input.phoneNumber=phone;
      }
      ...
      try{
        ...
        if(!upstream.ok){await upstream.body?.cancel();result={success:false,error:upstream.status===429?'The provider is busy. Try again later.':upstream.status>=500?'The provider could not be reached. Try again later.':'The provider could not verify this reference.'};}
        else{const normalized=normalizeVeritas(provider,outgoing.reference,await boundedJson(upstream));result=normalized.ok?{success:true,receipt:{provider,reference:normalized.data.reference,amount:normalized.data.amount,currency:normalized.data.currency,date:normalized.data.txnDate,receiverName:normalized.data.receiverName,receiverAccount:normalized.data.receiverAccount}}:{success:false,error:normalized.error};}
      }catch{result={success:false,error:'The provider did not respond in time. This lookup used one demo check.'};}
    ...
    }catch{return reply(503,{success:false,code:'DEMO_UNAVAILABLE',error:'Could not confirm the demo result. Check again before starting another lookup.'});}
  };
}
